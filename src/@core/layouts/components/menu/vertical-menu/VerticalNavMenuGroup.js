...
const VerticalNavMenuGroup = ({
  ...
  // ** Toggle Open Group
  const toggleOpenGroup = (itemLocal, parent) => {
    let openGroup = groupOpen
    const activeGroup = groupActive

    // ** If Group is already open and clicked, close the group
    if (openGroup.includes(itemLocal.id)) {
      openGroup.splice(openGroup.indexOf(itemLocal.id), 1)

      // ** If clicked Group has open group children, Also remove those children to close those groups
      if (itemLocal.children) {
        removeChildren(itemLocal.children, openGroup, groupActive)
      }
    } else if (activeGroup.includes(itemLocal.id) || currentActiveGroup.includes(itemLocal.id)) {
      ...
      if (!activeGroup.includes(itemLocal.id) && currentActiveGroup.includes(itemLocal.id)) {
        activeGroup.push(itemLocal.id)
      } else {
        activeGroup.splice(activeGroup.indexOf(itemLocal.id), 1)
      }
  ...
  // ** On Group Item Click
  const onCollapseClick = (e, itemLocal) => {
    toggleOpenGroup(itemLocal, parentItem)
