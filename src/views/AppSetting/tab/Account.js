...
import { appSettingUpdate, editProfile } from '../../../utility/apis/appSetting'
...
const Account = ({ info }) => {
  ...
  console.log(user?.websiteUrl)
  return (
    <>
      <Card>
        <CardHeader className='border-bottom'>
          <CardTitle tag='h4'>Profile Update</CardTitle>
        </CardHeader>
        <CardBody className='py-2 my-25'>
          <Form onSubmit={handleSubmit(onSubmit)}>
            ...
            <Row className='align-items-start gy-2 mt-1'>
              <Show IF={user?.userType === 0}>
                ...
                <Col md='4'>
                  <FormGroupCustom
                    type={'text'}
                    control={control}
                    name='file_gen_if_exceed'
                    value={info?.file_gen_if_exceed}
                    className='mb-1'
                    label='File Generation Exceeds'
                    errors={errors}
                    rules={{ required: true }}
                  />
                </Col>
