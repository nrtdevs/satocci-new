    ...
    if (Object.values(data).every((field) => field.length > 0)) {
      useJwt
        ...
        .then((res) => {
          ...
          dispatch(handleLogin(dataLocal))
          ability.update(res.data.userData.ability)
          navigate(getHomeRouteForLoggedInUser(dataLocal.role))
          toast((t) => (
            <ToastContent
              t={t}
              role={dataLocal.role || 'admin'}
              name={dataLocal.fullName || dataLocal.username || 'John Doe'}
            />
          ))
        })
