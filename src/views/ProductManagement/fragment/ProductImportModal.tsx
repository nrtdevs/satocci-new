...
import { useLoadCategoryHierarchyMutation } from '../../../redux/RTKQuery/CategoryRTK'
...
import { productSample } from '../../../utility/apis/ExportLanguage'
...
import { CategoryDropdown } from './ProductForm'
...
export default function ProductImportModal<T>(props: T & dataType) {
    const {
        edit = null,
        noView = false,
        active = false,
    ...
    ///IMPORT Product
    const [importProduct, resultData] = useImportProductMutation()
    ...
    const handleSave = () => {
        if (isValid(files)) {
            const re: any = {}
            const array = form.watch('categories')?.map((a: any) => a?.value)
            fastLoop(array, (a: any, index: number) => {
                re[`categories[${index}]`] = a
            })
            importProduct({
                // category_id: 3,
                ...re,
                // subcategory_id: 4,
                store_id: storeId ? storeId : userType === UserType.Store ? user?.id : null,
                file: files[0]
            })
        }
    }
                ...
                <CardBody className=''>
                    ...
                    <Row>
                        ...
                        <Col md='12' className='mb-1'>
                            ...
                            >
                                <div className='list-unstyled p-1'>{CategoryDropdown({ edit: null, form, parentId: null, level: 0, onSelect: (e) => { log('selected', e) }, hasChild: null, maxLevel: 0 })}</div>
