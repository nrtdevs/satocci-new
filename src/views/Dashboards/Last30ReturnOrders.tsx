...
import { useLoadReturnOrderMutation } from '../../redux/RTKQuery/ReturnOrderRTK'
...
const Last30ReturnOrders = ({ loading = false }: theProps) => {
    // ** vars
    ...
    useEffect(() => {
        // if (isValid(Can(Permissions.returnRefundBrowse))) {
        loadReturn({
            page: 1,
            per_page_record: 30
        })
    ...
    columns = [
...

        {
            ...
            cell: (row) => (
                <Link
                    state={{ row }}
                    to={getPath('admin.returns')}
                    className='d-block'
                    id='create-button'
                >
                    <span className='d-block fw-bold text-truncate'>
                        <u>#{row?.order?.order_number}</u>
                    </span>
                </Link>
            )
        },
            ...
            cell: (row) => (
                <span className='d-block fw-bold'>
                    {row?.order_item_returns?.map((a: any, i: any) => {
                        return <Badge color='light-primary' className='me-25 p-25' key={i}>{Number(a?.return_quantity).toFixed(1)}</Badge>
                    })}
                </span>
            )
        },
...
export default Last30ReturnOrders
